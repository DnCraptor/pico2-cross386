...
void x86_add_char_to_BDA(u16 keycode) {
    ...
    buffer[tail] = keycode;
    static int i = 0;
    goutf(30-1, false, "W %04X (%d)", keycode, i++);
    BDA16[1] = tail + 1;
}
...
__process_key(u8 scancode)
{
    ...
    case 0x54: /* SysReq */
        ...
        return;
    case 0x53: /* Del */
        if ((BDA->kbd_flag0 & (KF0_CTRLACTIVE | KF0_ALTACTIVE))
    ...
    if (!scancode || scancode >= sizeof(scan_to_keycode) / sizeof(struct scaninfo)) {
        goutf(30-1, true, "__process_key unknown scancode read: 0x%X", scancode);
        return;
    }
    ...
    if (keycode) {
        ...
        u16 buffer_head = BDA->kbd_buf_head;
        u16 buffer_tail = BDA->kbd_buf_tail;
        ...
        *(u16*)(BDA + temp_tail) = keycode;
        BDA->kbd_buf_tail = buffer_tail;
    }
}

void x86_int21_hanler_C(void) {
    //goutf(30-1, false, "W %02X", X86_PORTS[0x60]);
    __process_key(X86_PORTS[0x60]);
    X86_PORTS[0x62] &= ~2;
}
